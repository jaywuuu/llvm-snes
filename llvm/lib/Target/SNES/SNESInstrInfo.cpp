...
SNESInstrInfo::SNESInstrInfo(SNESSubtarget &ST)
    : SNESGenInstrInfo(SP::ADJCALLSTACKDOWN, SP::ADJCALLSTACKUP), RI(),
      Subtarget(ST) {}
...
unsigned SNESInstrInfo::insertBranch(
  ...
  assert((Cond.size() == 1 || Cond.size() == 0) &&
         "SNES branch conditions should have one component!");
...
Register SNESInstrInfo::getGlobalBaseReg(MachineFunction *MF) const {
  ...
  Register GlobalBaseReg = SNESFI->getGlobalBaseReg();
  if (GlobalBaseReg)
  ...
  SNESFI->setGlobalBaseReg(GlobalBaseReg);
  return GlobalBaseReg;
}
...
bool SNESInstrInfo::expandPostRAPseudo(MachineInstr &MI) const {
  ...
  case TargetOpcode::LOAD_STACK_GUARD: {
    ...
    // offsetof(tcbhead_t, stack_guard) from sysdeps/SNES/nptl/tls.h in glibc.
