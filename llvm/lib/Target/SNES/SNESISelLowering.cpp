...
static bool CC_SNES_Assign_SRet(unsigned &ValNo, MVT &ValVT,
...
static bool CC_SNES_Assign_Split_64(unsigned &ValNo, MVT &ValVT,
                                     MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                     ISD::ArgFlagsTy &ArgFlags, CCState &State)
{
...
static bool CC_SNES_Assign_Ret_Split_64(unsigned &ValNo, MVT &ValVT,
                                         MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                         ISD::ArgFlagsTy &ArgFlags, CCState &State)
{
  static const MCPhysReg RegList[] = {
...
static bool Analyze_CC_SNES64_Full(bool IsReturn, unsigned &ValNo, MVT &ValVT,
                                    MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                    ISD::ArgFlagsTy &ArgFlags, CCState &State) {
...
static bool Analyze_CC_SNES64_Half(bool IsReturn, unsigned &ValNo, MVT &ValVT,
                                    MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                    ISD::ArgFlagsTy &ArgFlags, CCState &State) {
...
static bool CC_SNES64_Full(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                            CCValAssign::LocInfo &LocInfo,
                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_SNES64_Full(false, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                 State);
}

static bool CC_SNES64_Half(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                            CCValAssign::LocInfo &LocInfo,
                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_SNES64_Half(false, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                 State);
}

static bool RetCC_SNES64_Full(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                               CCValAssign::LocInfo &LocInfo,
                               ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_SNES64_Full(true, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                 State);
}

static bool RetCC_SNES64_Half(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                               CCValAssign::LocInfo &LocInfo,
                               ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_SNES64_Half(true, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                 State);
}

#include "SNESGenCallingConv.inc"

// The calling conventions in SNESCallingConv.td are described in terms of the
...
bool SNESTargetLowering::CanLowerReturn(
  ...
  return CCInfo.CheckReturn(Outs, Subtarget->is64Bit() ? RetCC_SNES64
                                                       : RetCC_SNES32);
}
...
SNESTargetLowering::LowerReturn_32(SDValue Chain, CallingConv::ID CallConv,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_SNES32);
  ...
  if (MF.getFunction().hasStructRetAttr()) {
    SNESMachineFunctionInfo *SFI = MF.getInfo<SNESMachineFunctionInfo>();
...
SNESTargetLowering::LowerReturn_64(SDValue Chain, CallingConv::ID CallConv,
  ...
  // Analyze return values.
  CCInfo.AnalyzeReturn(Outs, RetCC_SNES64);
...
SDValue SNESTargetLowering::LowerFormalArguments_32(
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_SNES32);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i, ++InIdx) {
    ...
    if (Ins[InIdx].Flags.isSRet()) {
      if (InIdx != 0)
        report_fatal_error("SNES only supports sret on the first parameter");
      // Get SRet from [%fp+64].
    ...
    if (VA.getValVT() == MVT::i32 || VA.getValVT() == MVT::f32) {
      ...
      report_fatal_error("SNESv8 does not handle f128 in calls; "
                         "pass indirectly");
    } else {
...
SDValue SNESTargetLowering::LowerFormalArguments_64(
  ...
  // Analyze arguments according to CC_SNES64.
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_SNES64);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    assert(VA.isMemLoc());
    // The CC_SNES64_Full/Half functions compute stack offsets relative to the
    // beginning of the arguments area at %fp+BIAS+128.
    unsigned Offset = VA.getLocMemOffset() + ArgArea;
    unsigned ValSize = VA.getValVT().getSizeInBits() / 8;
    // Adjust offset for extended arguments, SNES is big-endian.
...
SNESTargetLowering::LowerCall_32(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(Outs, CC_SNES32);
  ...
  unsigned TF = isPositionIndependent() ? SNESMCExpr::VK_SNES_WPLT30
                                        : SNESMCExpr::VK_SNES_WDISP30;
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee))
  ...
  const SNESRegisterInfo *TRI = Subtarget->getRegisterInfo();
  const uint32_t *Mask =
      ((hasReturnsTwice)
  ...
  RVInfo.AnalyzeCallResult(Ins, RetCC_SNES32);
...
//
// The SNES v9 ABI requires that floating point arguments are treated the same
...
SNESTargetLowering::LowerCall_64(TargetLowering::CallLoweringInfo &CLI,
  ...
  // SNES target does not yet support tail call optimization.
  ...
  CCState CCInfo(CLI.CallConv, CLI.IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(CLI.Outs, CC_SNES64);

  // Get the size of the outgoing arguments stack space requirement.
  // The stack offset computed by CC_SNES64 includes all arguments.
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    case CCValAssign::BCvt:
      ...
      // SNES does not support i128 natively. Lower it into two i64, see below.
  ...
  RVInfo.AnalyzeCallResult(CLI.Ins, RetCC_SNES64);
...
/// IntCondCCodeToICC - Convert a DAG integer condition code to a SNES ICC
/// condition.
...
/// FPCondCCodeToFCC - Convert a DAG floatingp oint condition code to a SNES
/// FCC condition.
...
SNESTargetLowering::SNESTargetLowering(const TargetMachine &TM,
  ...
  if (Subtarget->is64Bit()) {
    ...
    // On 32bit SNES, we define a double-register 32bit register
  ...
  // SNES doesn't have i1 sign extending load
  ...
  // SNES doesn't have sext_inreg, replace them with shl/sra
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i16, Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i8 , Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i1 , Expand);

  // SNES has no REM or DIVREM operations.
  ...
  // SNES has no select or setcc: expand to SELECT_CC.
  ...
  // SNES doesn't have BRCOND either, it has BR_CC.
  ...
  // ATOMICs.
  // Atomics are supported on SNESV9. 32-bit atomics are also
  // supported by some Leon SNESV8 variants. Otherwise, atomics
  // are unsupported.
  if (Subtarget->isV9())
  ...
  if (!Subtarget->isV9()) {
    // SNESV8 does not have FNEGD and FABSD.
...
SDValue SNESTargetLowering::LowerGlobalTLSAddress(SDValue Op,
  ...
  if (model == TLSModel::GeneralDynamic || model == TLSModel::LocalDynamic) {
    unsigned HiTF = ((model == TLSModel::GeneralDynamic)
                     ? SNESMCExpr::VK_SNES_TLS_GD_HI22
                     : SNESMCExpr::VK_SNES_TLS_LDM_HI22);
    unsigned LoTF = ((model == TLSModel::GeneralDynamic)
                     ? SNESMCExpr::VK_SNES_TLS_GD_LO10
                     : SNESMCExpr::VK_SNES_TLS_LDM_LO10);
    unsigned addTF = ((model == TLSModel::GeneralDynamic)
                      ? SNESMCExpr::VK_SNES_TLS_GD_ADD
                      : SNESMCExpr::VK_SNES_TLS_LDM_ADD);
    unsigned callTF = ((model == TLSModel::GeneralDynamic)
                       ? SNESMCExpr::VK_SNES_TLS_GD_CALL
                       : SNESMCExpr::VK_SNES_TLS_LDM_CALL);
    ...
    SDValue Hi = DAG.getNode(SPISD::Hi, DL, PtrVT,
                 withTargetFlags(Op, SNESMCExpr::VK_SNES_TLS_LDO_HIX22, DAG));
    SDValue Lo = DAG.getNode(SPISD::Lo, DL, PtrVT,
                 withTargetFlags(Op, SNESMCExpr::VK_SNES_TLS_LDO_LOX10, DAG));
    HiLo =  DAG.getNode(ISD::XOR, DL, PtrVT, Hi, Lo);
    return DAG.getNode(SPISD::TLS_ADD, DL, PtrVT, Ret, HiLo,
                   withTargetFlags(Op, SNESMCExpr::VK_SNES_TLS_LDO_ADD, DAG));
  }

  if (model == TLSModel::InitialExec) {
    unsigned ldTF     = ((PtrVT == MVT::i64)? SNESMCExpr::VK_SNES_TLS_IE_LDX
                         : SNESMCExpr::VK_SNES_TLS_IE_LD);
    ...
    SDValue TGA = makeHiLoPair(Op,
                               SNESMCExpr::VK_SNES_TLS_IE_HI22,
                               SNESMCExpr::VK_SNES_TLS_IE_LO10, DAG);
    ...
    return DAG.getNode(SPISD::TLS_ADD, DL, PtrVT,
                       ...
                       withTargetFlags(Op,
                                       SNESMCExpr::VK_SNES_TLS_IE_ADD, DAG));
  }

  assert(model == TLSModel::LocalExec);
  SDValue Hi = DAG.getNode(SPISD::Hi, DL, PtrVT,
                  withTargetFlags(Op, SNESMCExpr::VK_SNES_TLS_LE_HIX22, DAG));
  SDValue Lo = DAG.getNode(SPISD::Lo, DL, PtrVT,
                  withTargetFlags(Op, SNESMCExpr::VK_SNES_TLS_LE_LOX10, DAG));
...
static SDValue LowerDYNAMIC_STACKALLOC(SDValue Op, SelectionDAG &DAG,
  ...
  if (Subtarget->is64Bit()) {
    ...
    // On SNES32, the size of the spill area is 92. Unfortunately,
...
// Custom lower UMULO/SMULO for SNES. This code is similar to ExpandNode()
