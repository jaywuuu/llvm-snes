...
static cl::opt<bool>
    ReserveAppRegisters("SNES-reserve-app-registers", cl::Hidden,
                        cl::init(false),
                        cl::desc("Reserve application registers (%g2-%g4)"));

SNESRegisterInfo::SNESRegisterInfo() : SNESGenRegisterInfo(SP::O7) {}
...
const uint32_t *
SNESRegisterInfo::getRTCallPreservedMask(CallingConv::ID CC) const {
...
void SNESRegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator II,
  ...
  const SNESSubtarget &Subtarget = MF.getSubtarget<SNESSubtarget>();
  const SNESFrameLowering *TFI = getFrameLowering(MF);
...
// SNES has no architectural need for stack realignment support,
// except that LLVM unfortunately currently implements overaligned
// stack objects by depending upon stack realignment support.
// If that ever changes, this can probably be deleted.
bool SNESRegisterInfo::canRealignStack(const MachineFunction &MF) const {
  if (!TargetRegisterInfo::canRealignStack(MF))
    return false;

  // SNES always has a fixed frame pointer register, so don't need to
